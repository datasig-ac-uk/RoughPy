...
)
{
    ...
    for (const auto& [lkey, li] : left) {
        for (const auto& [rkey, ri] : right) {
            for (const auto& [okey, oscal] : key_fn(lkey, rkey)) {
                scalar_fn(out_map[okey], oscal * left_data[li], right_data[ri]);
            }
        }
    }
}
...
)
{
    ...
    for (const auto& [lkey, li] : left) {
        ...
        for (const auto& [rkey, ri] : levels[rdegree]) {
            for (const auto& [okey, oscal] : key_fn(lkey, rkey)) {
                scalar_fn(mapped[okey], oscal * lscal, right_data[ri]);
            }
        }
    }
}
...
{
    containers::Vec<ranges::subrange<
            ranges::iterator_t<RightRng>,
            ranges::sentinel_t<RightRng>>>
            levels;
    levels.reserve(max_degree + 1);
    auto first = ranges::begin(right);
    deg_t last_degree = 0;
    for (auto it = ranges::begin(right); it != ranges::end(right); ++it) {
        const auto degree = basis->degree(it->first);
        if (degree > max_degree) { break; }
        for (; last_degree < degree; ++last_degree) {
            levels.emplace_back(first, it);
            first = it;
        }
    }
    {
        const auto end = ranges::end(right);
        for (; last_degree <= max_degree; ++last_degree) {
            levels.emplace_back(end, end);
        }
    }
    ...
    for (const auto& [lkey, li] : left) {
        ...
        for (const auto& [rkey, ri] : levels[rdegree]) {
            ...
            for (const auto& [okey, oscal] : key_fn(lkey, rkey)) {
                scalar_fn(mapped[okey], oscal * lscal, rscal);
            }
        }
    }
}
