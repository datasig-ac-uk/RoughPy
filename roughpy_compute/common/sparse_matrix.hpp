...
public:
    ...
    struct Sentinel {
        ...
        constexpr Sentinel(difference_type end_index) noexcept
...
public:
    constexpr CompressedRangeIterator(
            DataIter_ data,
    ...
    friend constexpr bool operator==(
            ...
            CompressedRangeIterator const& rhs
    ) noexcept
    {
    ...
    operator==(CompressedRangeIterator const& lhs, Sentinel const& rhs) noexcept
    {
...
public:
    ...
    constexpr CompressedRange(
            DataIter_ data,
            IndicesIter_ indices,
            difference_type count
    )
        : begin_(std::move(data), std::move(indices)),
          end_(count)
    {}
...
inline constexpr auto CompressedRow = CompressedDim::Rows;
inline constexpr auto CompressedCol = CompressedDim::Cols;

template <
        typename DataIter_,
        typename OffsetIter_,
        typename IndicesIter_,
        CompressedDim CompressedDim_ = CompressedRow>
class CompressedMatrix
{
     ...
     * n_cols (+1) offsets into the row indices and data ranges. Thus the
     * "inner" dimension refers to the dimension that is given indices: for csr
     * the inner dim is columns, and for csc the inner dim is rows.
     */
...
public:
    ...
    constexpr compressed_range in_dim(Index dim) const noexcept
    {
        ...
        return {data_ + dim_begin, indices_ + dim_begin, dim_end - dim_begin};
    }
