...
ffi::Error cpu_dense_ft_log_impl(
    ...
    if (result_dim != arg_dim) {
        return ffi::Error::InvalidArgument("cpu_dense_ft_log result array must match out array");
    }

    if (result_size != arg_size) {
        return ffi::Error::InvalidArgument("cpu_dense_ft_log result size must match out size");
    }
