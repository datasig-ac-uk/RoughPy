#ifndef ROUGHPY_DEVICE_SUPPORT_LOWER_BOUND_H_
#define ROUGHPY_DEVICE_SUPPORT_LOWER_BOUND_H_
...
struct Func : dtl::AlgorithmFunctionBase<S, T, Cond> {
    ...
    operator()(const Buffer& buffer, ConstReference value) const
    {
        ...
        auto bound = rpy::ranges::lower_bound(buffer_slice, bound_val);
...
template <typename S, typename T>
using func = Func<S, T, is_less_comparable_v<S, T>>;
}// namespace __lower_bound

template <typename S, typename T>
constexpr __lower_bound::func<S, T> lower_bound_func{};

}// namespace devices
}// namespace rpy

#endif// ROUGHPY_DEVICE_SUPPORT_LOWER_BOUND_H_
