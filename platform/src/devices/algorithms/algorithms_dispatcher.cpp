...
void AlgorithmsDispatcher::default_fill(Buffer& buffer) const
{
    auto& impl = this->get_implementor(buffer.type(), buffer.type());
    impl.default_fill(buffer);
}
...
{
    auto& impl = this->get_implementor(left.type(), right.type());
    return impl.mismatch(left, right);
}

void AlgorithmsDispatcher::copy(Buffer& dest, const Buffer& source) const
{
    auto& impl = this->get_implementor(dest.type(), source.type());
