...
RPY_NO_EXPORT PyObject* outer_loop_ternary(
        PyArrayObject* out,
        PyArrayObject* lhs,
        PyArrayObject* rhs,
        Fn&& fn,
...
)
{
    ...
    const npy_intp ndims = PyArray_NDIM(out);
    ...
    for (npy_intp i = 0; i < n_elements; ++i, advance()) {
        ...
        if (out_stride == 1 && lhs_stride == 1 && rhs_stride == 1) {
            RPY_STATUS_OR_RETURN_NULL(fn(ctx, out_ptr, lhs_ptr, rhs_ptr));
        } else {
...
[[gnu::always_inline]] RPY_NO_EXPORT inline PyObject* ternary_function_outer(
        PyObject* out_obj [[maybe_unused]],
        PyObject* lhs_obj,
        PyObject* rhs_obj,
        CallConfig& config
)
