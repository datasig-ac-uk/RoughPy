...
RPY_NO_EXPORT PyObject* outer_loop_binary(
        PyArrayObject* out,
        PyArrayObject* arg,
        Fn&& fn,
        const Ctx& ctx
)
{
    ...
    for (npy_intp i = 0; i < ndims; ++i) { index[i] = 0; }
    ...
    for (npy_intp i = 0; i < n_elements; ++i, advance()) {
        ...
        auto const* arg_ptr
                = static_cast<Scalar const*>(PyArray_GetPtr(arg, index.data()));
        ...
        if (out_stride == 1 && arg_stride == 1) {
            RPY_STATUS_OR_RETURN_NULL(fn(ctx, out_ptr, arg_ptr));
        } else {
            RPY_STATUS_OR_RETURN_NULL(
                    fn(ctx,
...
[[gnu::always_inline]] RPY_NO_EXPORT inline PyObject*
binary_function_outer(PyObject* out_obj, PyObject* arg_obj, CallConfig& config)
{
