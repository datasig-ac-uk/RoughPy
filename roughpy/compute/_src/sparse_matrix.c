...
PyMemberDef PySparseMatrix_members[] = {
        ...
        {"indices",
         ...
         0, "indices"},
        {"indptr", Py_T_OBJECT_EX, offsetof(PySparseMatrix, indptr), 0, "indptr"
        },
        {"rows", Py_T_PYSSIZET, offsetof(PySparseMatrix, rows), READONLY, "rows"
        },
        {"cols", Py_T_PYSSIZET, offsetof(PySparseMatrix, cols), READONLY, "cols"
        },
        {NULL}
};
...
PyMethodDef PySparseMatrix_methods[] = {
        {"getformat",
         ...
         METH_O, "get the string representation of the matrix format"},
        {NULL}
};
...
)
{
    PySparseMatrix* self
            = (PySparseMatrix*)
                      PySparseMatrix_Type.tp_alloc(&PySparseMatrix_Type, 0);
    if (!self) {
        Py_XDECREF(data);
        Py_XDECREF(indices);
        Py_XDECREF(indptr);
