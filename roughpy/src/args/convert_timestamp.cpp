...
rpy::param_t pytimedelta_to_param(PyObject* py_time_delta, const options_t& options) {
    ...
    if (PyDelta_Check(py_time_delta)) {
        ...
        auto* normalised_delta = PyNumber_TrueDivide(py_time_delta, py_reference);
        if (normalised_delta == nullptr) {
...
py::object generic_to_timestamp(py::handle generic_timestamp) {
    ...
    if (PyDateTime_Check(generic_timestamp.ptr()) || PyTime_Check(generic_timestamp.ptr())) {
    ...
    if (PyUnicode_Check(generic_timestamp.ptr())) {
        throw py::type_error("currently conversion from ISO-8601 strings is not supported");
    }
...
rpy::param_t rpy::python::convert_delta_from_datetimes(py::handle current, py::handle previous, const rpy::python::PyDateTimeConversionOptions &options) {
    ...
    if (py::isinstance<py::float_>(current) && py::isinstance<py::float_>(previous)) {
