...
function(_check_runtime_unknown_component _library _out_var)
    get_target_property(_imported_libs ${_library} IMPORTED_LOCATION)

    set(_deps)
    foreach (_loc ${_imported_libs})
        cmake_path(GET _loc EXTENSION _ext)
        message(${_loc})
        if (_ext MATCHES ${CMAKE_SHARED_LIBRARY_SUFFIX})
            cmake_path(GET _loc FILENAME _fname)
            message(STATUS "Adding runtime dependency ${_fname}")
            list(APPEND _deps ${_loc})
        endif()
    endforeach()
        ...
        foreach (_dep IN LISTS _list)
            if (NOT _dep MATCHES RoughPy)
                ...
                elseif(_type STREQUAL "UNKNOWN_LIBRARY")
                    _check_runtime_unknown_component(${_name} _this_dep)
                    message(STATUS "Runtime dependency added: ${_name}")
                    list(APPEND _these_deps ${_this_dep})
                endif ()
